...
import AdHistory from './MarketingHistory';
...
vi.mock('./GeneratedMarketingCard', () => ({
    default: ({ onDownload, onDelete, marketing }) => (
        <div>
            <button onClick={() => onDownload(marketing.id, marketing.headline)}>download</button>
...
describe('AdHistory', () => {
    ...
    it('renders HistoryList with the correct props', () => {
        ...
        expect(screen.getByTestId('api-url')).toHaveTextContent('/api/marketings');
    ...
    it('downloads an marketing with the correct URL and filename', () => {
        render(<AdHistory refreshKey={1} />);

        fireEvent.click(screen.getByRole('button', { name: 'download' }));

        expect(mockDownloadFile).toHaveBeenCalledWith('http://localhost:3000/api/marketings/1/download', 'Nyári akció.png');
    });

    it('falls back to a generated filename when the marketing has no headline', () => {
        ...
        expect(mockDownloadFile).toHaveBeenCalledWith(
            'http://localhost:3000/api/marketings/42/download',
            'hirdetes-42.png'
        );
    });
    ...
    it('falls back to a generated filename when the headline is only whitespace', () => {
        ...
        expect(mockDownloadFile).toHaveBeenCalledWith(
            'http://localhost:3000/api/marketings/43/download',
            'hirdetes-43.png'
        );
    });
});